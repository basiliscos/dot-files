...
beautiful.init("/home/b/.config/awesome/themes/default/theme.lua")
...
local cpu = lain.widget.cpu {
    settings = function()
        local perc_markup = markup.fg.color('#FFFFFF', string.format('%03d%%', cpu_now.usage))
        local cpu_markup = string.format("[cpu: %s]", perc_markup)
        widget:set_markup(cpu_markup)
    end
}
local memory = lain.widget.mem {
    settings = function()
        local color = (mem_now.perc > 75) and '#FFFF00' or '#FFFFFF'
        local mem_markup = markup.fg.color(color, string.format('%03d%%', mem_now.perc))
        widget:set_markup(string.format("[ram: %s]", mem_markup))
    end
}
local mynetdown = lain.widget.net {
    units = 1048576,
    settings = function()
...
local battery = lain.widget.bat {
    ...
    n_perc = {10, 30},
    settings = function()
        local ac = (bat_now.status == 'Full') and {'⚑', '#00FF00'}
            or (bat_now.status == 'Charging') and {'⚡', '#00DD00'}
            or {'⚑', '#FFFF00' }
        local ac_markup = markup.fg.color(ac[2], ac[1])
        local perc_markup = markup.fg.color('#FFFFFF', string.format("%0.3d%%", bat_now.perc))
        widget:set_markup(string.format("[batt: %s %s]", perc_markup, ac_markup))
    end
}
local ip_addr = wibox.widget{
    markup = '[?]',
    align  = 'center',
    valign = 'center',
...
mytextclock = wibox.widget.textclock("%d/%m(%a) %H:%M:%S", 1)

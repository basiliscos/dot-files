...
case "$1" in
    ...
    button/lid)
        case "$3" in
            close)
                # suspend-to-ram
                logger "LID closed, setting min speed"
                printf '%s' "$minspeed" > "$setspeed"
                XAUTHORITY=/home/b/.Xauthority DISPLAY=:0 xset dpms force off
                # zzz
                ;;
            open)
                logger "LID opened, setting max speed"
                printf '%s' "$maxspeed" >"$setspeed"
                XAUTHORITY=/home/b/.Xauthority DISPLAY=:0 xset dpms force on
                ;;
